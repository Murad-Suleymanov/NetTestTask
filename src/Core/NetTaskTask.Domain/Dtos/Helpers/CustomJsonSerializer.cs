...
namespace NetTestTask.Utility.Helpers
{
    public class CustomJsonSerializer
    {
        public CustomJsonSerializer(Type type)
        {
            _type = type;
        }
        ...
        public object Deserialize(string des)
        {
            ...
            for (int i = 0; i < pairs.Count; i++)
            {
                ...
                if (!property.PropertyType.IsValueType && property.PropertyType != typeof(DateTime) && property.PropertyType != typeof(string) && property.PropertyType.Name.IndexOf("Nullable") < 0)
                {
                    ...
                    CustomJsonSerializer childSamurSoft = new CustomJsonSerializer(type);
        ...
        public string Serialize(object graph)
        {
            ...
            for (int i = 0; i < properties.Count; i++)
            {
                ...
                else
                {
                    ...
                    CustomJsonSerializer serializer = new CustomJsonSerializer(property.GetType());
                    serializedText += String.Format("{0}:{1}", properties[i].Name, serializer.Serialize(property));
                }
                serializedText += ",";
            }
            serializedText += "}";
